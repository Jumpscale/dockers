...
ays install -n agentcontroller2_client --consume 'ac/agentcontroller2!main'
...
#restart nginx
ays install -n agentcontroller2_keys --data "instance.country:AE instance.state:Dubai instance.locality:Dubai instance.organisation:GIG instance.commonname:ac" \
--consume 'ac/agentcontroller2!main,ws/nginx!main'


# SSL doesn't seems to work IP address, some we need to use dns
ays execute --cmd "echo "94.23.38.89" ac >> /etc/hosts" -n node.ssh -i ovh4
...
ays install -n agent2  -i ovh4 --consume 'ac/agentcontroller2!main,node/node.ssh!ovh4' --data 'instance.agentcontroller:https://ac/controller/ instance.gid:1 instance.nid:1 instance.roles:agent'


#configure agents to use redis internally
# ays install -n agent2_stats -i ovh4 --consume 'agent/agent2!node1'
